...
function ProductListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface[]>(
    ...
    () =>
      getProducts({
        relations: ['shop_system', 'order_item.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  <Th>inventory</Th>
                  <Th>category</Th>
                  {hasAccess('shop_system', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>shop_system</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('shop_system', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/shop-systems/view/${record.shop_system?.id}`}>
                          {record.shop_system?.name}
                        </Link>
                      </Td>
                    )}
